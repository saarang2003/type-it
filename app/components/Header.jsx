...
export default function Header({ windowWidth, roomCode, onLogoClick, onLeaveRoom }: Props) {
  ...
  return (
    <header
      className={`
        flex justify-between items-center px-8 py-8 overflow-hidden
        transition-opacity
    ...
    >
      <div className="flex items-center w-full">
        ...
        <div
          className={`
            flex relative ml-[30px] pl-[10px] w-full
            transition-opacity
            ${typingFocused ? "opacity-0 pointer-events-none" : "opacity-100"}
            max-[800px]:ml-[15px]
            max-[650px]:ml-[10px] max-[650px]:pl-[5px]
            before:content-[''] before:absolute before:left-0 before:top-1/2
            before:-translate-y-1/2 before:h-[75%] before:w-[2px]
            before:bg-[var(--clr-char)] before:rounded-full
          `}
        >
          <ButtonRounded
          ...
          {!roomCode ? (
            ...
            >
              <Icon1v1 className="w-[20px] h-[20px]" />
